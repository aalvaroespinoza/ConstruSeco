...
class PestanaNuevaVenta(QWidget):
    ...
    def migrar_esquema(self):
        ...
        if columnas_conversion and "operacion" not in columnas_conversion:
            cursor.execute(
                "ALTER TABLE conversiones_unidad "
                "ADD COLUMN operacion TEXT NOT NULL DEFAULT 'MULTIPLICAR'"
            )
        columnas_nuevas = [
        ...
        for nombre, tipo in columnas_nuevas:
            if nombre not in columnas_documentos:
